...
This research document provides a technical analysis of Bongacams video detection and download workflows using evidence collected from the supplied CSV row, a bounded homepage fetch, a non-download `yt-dlp` probe, local yt-dlp extractor-source inspection, and official tool documentation. The target domain for this row is `serp.ly`.
...
2. [Bongacams Evidence Summary](#2-bongacams-evidence-summary)
...
Site name: Bongacams
Domain: serp.ly
Article path: content/site-articles/serp-ly.md
CTA URL: https://serp.ly/bongacams-downloader?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent
...
```text
Attempted URL: https://serp.ly/?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent
Final URL: https://serp.ly/?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent
HTTP status: 200
...
yt-dlp homepage stderr: ERROR: Unsupported URL: https://serp.ly/?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent
yt-dlp homepage duration_seconds: 0.83
```


Developer takeaway: run yt-dlp against the actual user-provided watch URL before deciding whether Bongacams is directly supported. Homepage success can indicate a working extractor or generic parser, while homepage failure can simply mean the homepage is not a video page.

Recommended verification commands:

```bash
URL="https://serp.ly/path/to/video?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent"
...
No permanent CDN host is asserted for Bongacams unless a future watch-page probe observes one. Record hosts for diagnostics, but design retries around re-detection rather than hard-coded CDN templates.
...
```bash
page_url="https://serp.ly/path/to/video?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent"
...
```bash
ffmpeg -i "input-from-serp-ly.ext" -map 0 -c copy -movflags +faststart "serp-ly-fixed.mp4"
```
...
1. Open the actual Bongacams watch page in the user's authorized browser session.
