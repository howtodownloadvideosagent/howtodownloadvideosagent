...
Site name: SexVid
Hostname: sexvid.ws
Output article path: content/site-articles/sexvid-ws.md
...
For SexVid, treat `sexvid.ws` and `www.sexvid.ws` as first-party hostnames during page discovery. The detector should not hard-code any third-party CDN domain for SexVid. Instead, it should build a per-page evidence set from these places:

- Static HTML returned for the accessible `sexvid.ws` page
...
```js
const allowedHosts = new Set(["sexvid.ws", "www.sexvid.ws"]);
...
const mediaEvidence = {
  ...
  scripts: [...document.scripts].map(script => script.src || script.textContent || "")
    .filter(value => /sexvid\.ws|jwplayer|videojs|kt_player|flashvars|m3u8|mpd|mp4|webm/i.test(value))
};
...
For SexVid, the downloader should look for common generic player shapes without claiming they are always present on `sexvid.ws`:
...
The detector should preserve the original `sexvid.ws` page URL as the `Referer` for any iframe, manifest, or direct media request unless runtime evidence proves a different referer is required.
...
If the video element shows `blob:https://sexvid.ws/...`, do not try to download the blob URL. Browser blob URLs are local object references. Inspect network requests for the underlying `.m3u8`, `.mpd`, `.m4s`, `.ts`, `.mp4`, or `.webm` requests.
...
```bash
yt-dlp   --cookies-from-browser chrome   --add-headers "Referer:$page_url"   --add-headers "Origin:https://sexvid.ws"   --add-headers "User-Agent:Mozilla/5.0"   --use-extractors generic   --list-formats   "$page_url"
```
...
```bash
yt-dlp   --add-headers "Referer:$page_url"   --add-headers "Origin:https://sexvid.ws"   -f "bv*+ba/b"   --merge-output-format mp4   "$media_url"
```
...
'"Origin: https://sexvid.ws"$'
'   -i "$media_url"   -c copy   sexvid-ws-output.mp4
```
...
```bash
ffmpeg   -headers "Referer: $page_url"$'
'   -i "$media_url"   -c copy   sexvid-ws-direct-output.mp4
```
...
```bash
ffmpeg -i sexvid-ws-output.ts -c copy sexvid-ws-output.mp4
ffmpeg -i sexvid-ws-output.mkv -c copy sexvid-ws-output.mp4
```
...
```bash
ffmpeg   -i sexvid-ws-output.webm   -c:v libx264   -c:a aac   -movflags +faststart   sexvid-ws-transcoded.mp4
```
...
```bash
streamlink   --http-header "Referer=$page_url"   "$media_url"   best -o sexvid-ws-output.ts
```
...
```bash
aria2c   --header="Referer: $page_url"   --out=sexvid-ws-output.mp4   "$media_url"
```
...
1. Match `sexvid.ws` and `www.sexvid.ws` page URLs.
