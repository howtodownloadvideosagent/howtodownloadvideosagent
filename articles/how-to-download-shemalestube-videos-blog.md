...
This research document provides a technical analysis of shemalestube video detection and download workflows using evidence collected from the supplied CSV row, a bounded homepage fetch, a non-download `yt-dlp` probe, local yt-dlp extractor-source inspection, and official tool documentation. The target domain for this row is `shemaletube.tv`.
...
shemalestube Downloader is a browser extension built for users who want a cleaner way to save accessible shemalestube videos without falling back to developer tools, network tabs, or command-line utilities. It detects supported media sources in the active browser session, preserves request context, and exports a local file when the stream is technically supported.
...
2. [shemalestube Evidence Summary](#2-shemalestube-evidence-summary)
...
## 2. shemalestube Evidence Summary
...
Domain: shemaletube.tv
Article path: content/site-articles/shemaletube-tv.md
...
```text
Attempted URL: https://shemaletube.tv/
Final URL: https://www.shemaletube.tv/
HTTP status: 200
Content-Type: text/html;charset=utf-8
Bytes read: 250000
Fetch error: none observed
HTML title: Free Tranny Porn Movies &amp; Shemale Sex Videos
```
...
yt-dlp homepage stderr: ERROR: Unsupported URL: https://www.shemaletube.tv/
yt-dlp homepage duration_seconds: 0.96
```


Developer takeaway: run yt-dlp against the actual user-provided watch URL before deciding whether shemalestube is directly supported. Homepage success can indicate a working extractor or generic parser, while homepage failure can simply mean the homepage is not a video page.

Recommended verification commands:

```bash
URL="https://shemaletube.tv/path/to/video"
...
No permanent CDN host is asserted for shemalestube unless a future watch-page probe observes one. Record hosts for diagnostics, but design retries around re-detection rather than hard-coded CDN templates.
...
```bash
page_url="https://shemaletube.tv/path/to/video"
...
```bash
ffmpeg -i "input-from-shemaletube-tv.ext" -map 0 -c copy -movflags +faststart "shemaletube-tv-fixed.mp4"
```
...
1. Open the actual shemalestube watch page in the user's authorized browser session.
