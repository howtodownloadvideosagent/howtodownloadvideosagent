# How to Download 4tube Videos | Technical Analysis of Stream Patterns, Extractor Behavior, and Download Methods

This research document provides a technical analysis of 4tube video extraction, including local yt-dlp support evidence, URL recognition, stream discovery behavior, format inspection, FFmpeg processing, and browser/HAR diagnostics. The evidence comes from the installed yt-dlp `2025.12.08` extractor source and local extractor inventory, not from visiting explicit media pages or downloading media.

4tube has direct yt-dlp support. Local `yt-dlp --list-extractors` on version `2025.12.08` lists `4tube`, and the inspected extractor source defines URL matching and media discovery for `4tube.com`.
The class is `FourTubeIE` and it inherits `FourTubeBaseIE`.

Its source-specific settings are `_URL_TEMPLATE = https://www.4tube.com/videos/%s/video` and `_TKN_HOST = token.4tube.com`.

But first...

## 4tube Video Downloader Chrome Extension

If you prefer a simpler one-click method, check out the 4tube Video Downloader browser extension.

4tube Downloader is a browser extension built for users who want a cleaner way to save accessible 4tube videos without falling back to developer tools, network tabs, or command-line utilities. It works from the active browser session, detects supported media sources when they are exposed to the page, and exports the final result as a usable local file for offline playback.

- Save supported 4tube videos from pages you are authorized to access
...
👉 Click here to try it free: https://serp.ly/4tube-downloader?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent
...
2. [4tube Video Infrastructure Overview](#2-4tube-video-infrastructure-overview)
...
The key implementation point is to treat yt-dlp's extractor output as the source of truth. For 4tube, the inspected source defines how to recognize supported URLs, how to move from a page URL to media candidates, and which protocol parser is used for each candidate. That is much safer than copying one observed media URL and building assumptions around its host.
...
## 2. 4tube Video Infrastructure Overview

The `4tube` path uses `FourTubeBaseIE`, a shared extractor base that first downloads the canonical video page and then discovers a media ID plus a list of available source qualities.
The source-backed token host is `token.4tube.com`. The base extractor posts to that host with `Origin` derived from the page URL and `Referer` set to the canonical video page. The response maps each requested quality to a tokenized media URL.
...
Those facts are more useful than a broad CDN claim. A robust downloader should store the original 4tube URL, the extractor name, the selected format ID, the protocol, and any HTTP headers returned by yt-dlp. It should not store a copied media URL as the only durable reference.
...
```text
https://www.4tube.com/videos/123456/display-slug
```
```text
https://m.4tube.com/videos/123456/display-slug
```
```text
https://www.4tube.com/embed/123456
```
...
Useful detection signals for 4tube diagnostics:
...
Conservative CDN conclusions for 4tube:
...
read -r HAR_FILE
jq -r '.log.entries[] | [.request.method, .request.url, .response.status] | @tsv' "$HAR_FILE" | rg -i 'token.4tube.com|data-quality|data-id|playerembed|desktop|.mp4|.m3u8|.mpd|.mp4|.webm'
```
For batch jobs, keep an archive and throttle concurrency so failures are diagnosable:

```bash
read -r URL_LIST
yt-dlp --batch-file "$URL_LIST" --download-archive 4tube-com-archive.txt --sleep-requests 1 --sleep-interval 2 --max-sleep-interval 5
```
...
ffmpeg -hide_banner -loglevel warning \
  ...
  -i "$MEDIA_URL" \
  -c copy -bsf:a aac_adtstoasc 4tube-com-hls.mp4
```
...
Good backup methods for 4tube are evidence-gathering tools:
...
Recommended downloader flow for 4tube:
...
Common 4tube troubleshooting sequence:
