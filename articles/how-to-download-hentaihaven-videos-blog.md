...
```text
Canonical hostname: hentaihaven.com
Same-site URL prefix: https://hentaihaven.com/
Hostname regex: ^(?:www\.)?hentaihaven\.com$
Iframe/src host check: new URL(src).hostname is hentaihaven.com or another explicitly discovered player host
```

The conservative architecture model is:

1. Start with the authorized Hentaihaven page on `https://hentaihaven.com/`.
...
```regex
https?:\/\/(?:www\.)?hentaihaven\.com(?:[\/?#][^\s"'<>]*)?$
```

Browser-side scan:

```javascript
const siteHost = "hentaihaven.com";
...
```bash
printf '%s
' "$HENTAIHAVEN_COM_MEDIA_URL" | node -e '
...
Start with the original authorized page URL. The generic extractor can sometimes find embedded players and media sources even when there is no named extractor. In these examples, `HENTAIHAVEN_COM_PAGE_URL` is set by the caller to the current authorized Hentaihaven page URL.

```bash
yt-dlp -F "$HENTAIHAVEN_COM_PAGE_URL"
yt-dlp --dump-json --skip-download "$HENTAIHAVEN_COM_PAGE_URL"
yt-dlp --force-generic-extractor -F "$HENTAIHAVEN_COM_PAGE_URL"
```
...
yt-dlp \
  ...
  --add-headers "Referer:$HENTAIHAVEN_COM_PAGE_URL" \
  --add-headers "User-Agent:Mozilla/5.0" \
  -F "$HENTAIHAVEN_COM_PAGE_URL"
```
...
```bash
ffmpeg \
  -headers "Referer: $HENTAIHAVEN_COM_PAGE_URL\r\nUser-Agent: Mozilla/5.0\r\n" \
  -protocol_whitelist file,http,https,tcp,tls,crypto \
  -i "$HENTAIHAVEN_COM_MANIFEST_URL" \
  -map 0 \
  -c copy \
  hentaihaven-com.mp4
```
...
```bash
ffmpeg -i "$HENTAIHAVEN_COM_MEDIA_URL" -map 0 -c copy hentaihaven-com.mkv
ffmpeg -i "$HENTAIHAVEN_COM_MEDIA_URL" -c:v libx264 -crf 20 -preset veryfast -c:a aac hentaihaven-com-compatible.mp4
```
...
```bash
ffprobe -hide_banner "$HENTAIHAVEN_COM_MEDIA_URL"
curl -I -L -H "Referer: $HENTAIHAVEN_COM_PAGE_URL" "$HENTAIHAVEN_COM_MEDIA_URL"
```
...
```bash
yt-dlp --verbose --force-generic-extractor -F "$HENTAIHAVEN_COM_PAGE_URL"
yt-dlp --cookies-from-browser chrome --verbose -F "$HENTAIHAVEN_COM_PAGE_URL"
ffprobe -hide_banner "$HENTAIHAVEN_COM_MANIFEST_URL"
```
