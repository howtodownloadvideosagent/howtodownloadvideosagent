# How to Download RedTube Videos | Technical Analysis of Stream Patterns, Extractor Behavior, and Download Methods

This research document provides a technical analysis of RedTube video extraction, including local yt-dlp support evidence, URL recognition, stream discovery behavior, format inspection, FFmpeg processing, and browser/HAR diagnostics. The evidence comes from the installed yt-dlp `2025.12.08` extractor source and local extractor inventory, not from visiting explicit media pages or downloading media.

RedTube has direct yt-dlp support. Local `yt-dlp --list-extractors` on version `2025.12.08` lists `RedTube`, and the inspected extractor source defines URL matching and media discovery for `redtube.com`.
The extractor detects removed and private video states before building formats.

It reads `sources` JSON, `mediaDefinition` arrays, HLS JSON expansions, and a final HTML5 `<source>` fallback.

But first...

## RedTube Video Downloader Chrome Extension

If you prefer a simpler one-click method, check out the RedTube Video Downloader browser extension.

RedTube Downloader is a browser extension built for users who want a cleaner way to save accessible RedTube videos without falling back to developer tools, network tabs, or command-line utilities. It works from the active browser session, detects supported media sources when they are exposed to the page, and exports the final result as a usable local file for offline playback.

- Save supported RedTube videos from pages you are authorized to access
...
👉 Click here to try it free: https://serp.ly/redtube-downloader?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent
...
2. [RedTube Video Infrastructure Overview](#2-redtube-video-infrastructure-overview)
...
The key implementation point is to treat yt-dlp's extractor output as the source of truth. For RedTube, the inspected source defines how to recognize supported URLs, how to move from a page URL to media candidates, and which protocol parser is used for each candidate. That is much safer than copying one observed media URL and building assumptions around its host.
...
RedTube extraction downloads the canonical numeric video page and checks explicit removed/private markers before parsing formats.
The source reads JSON-LD first, then looks for a `sources` object, a `mediaDefinition` array, optional follow-up JSON for HLS or MP4 media definitions, and finally an HTML5 `<source>` fallback.
That layered behavior is source-backed evidence for direct and HLS handling, but not for a single durable CDN hostname.

For implementation planning, the extractor source gives four concrete pieces of evidence:

- support: dedicated-layered-page-extractor
- stream_types: hls-and-direct-mp4
- metadata_source: json-ld-and-page-fields
- retry_policy: re-fetch-page-and-media-json

Those facts are more useful than a broad CDN claim. A robust downloader should store the original RedTube URL, the extractor name, the selected format ID, the protocol, and any HTTP headers returned by yt-dlp. It should not store a copied media URL as the only durable reference.
...
```
```text
https://embed.redtube.com/?id=1443286
```
```text
https://www.redtube.com.br/103224331
```

The exact regular expression lives in `redtube.py`. In application code, prefer passing the original URL to yt-dlp instead of maintaining a parallel regex. If pre-validation is required, keep it permissive enough to avoid rejecting URLs that yt-dlp would accept.

Useful detection signals for RedTube diagnostics:
...
Direct source-object entries are returned with height derived from the format ID when possible. HLS definitions are expanded with yt-dlp's M3U8 parser. Other media definitions become direct MP4 formats.
Because the source may need to download more media JSON from a `videoUrl`, applications should avoid assuming the first page contains every final URL.
If no structured formats exist, the extractor falls back to an HTML5 MP4 source. That fallback is useful, but it should not be the only implementation path.

Conservative CDN conclusions for RedTube:
...
read -r HAR_FILE
jq -r '.log.entries[] | [.request.method, .request.url, .response.status] | @tsv' "$HAR_FILE" | rg -i 'sources:|mediaDefinition|videoUrl|.m3u8|<source src=|.m3u8|.mpd|.mp4|.webm'
```
For batch jobs, keep an archive and throttle concurrency so failures are diagnosable:

```bash
read -r URL_LIST
yt-dlp --batch-file "$URL_LIST" --download-archive redtube-com-archive.txt --sleep-requests 1 --sleep-interval 2 --max-sleep-interval 5
```
...
ffmpeg -hide_banner -loglevel warning \
  ...
  -i "$MEDIA_URL" \
  -c copy -bsf:a aac_adtstoasc redtube-com-hls.mp4
```
...
Good backup methods for RedTube are evidence-gathering tools:
...
Recommended downloader flow for RedTube:
...
Common RedTube troubleshooting sequence:
...
- Removed and private markers should be shown as expected content states.
- Some media definitions point to more JSON rather than final media.
- Embed URLs resolve through the same numeric ID workflow.

When those checks still fail, keep the report factual: include yt-dlp version `2025.12.08`, extractor name `RedTube`, the URL shape, whether cookies were used, whether the failure happened at page download, player parsing, API call, manifest parsing, or FFmpeg processing, and the exact expected content-state message if yt-dlp provided one.

## 10. Sources

- Local yt-dlp evidence: `yt-dlp --version` returned `2025.12.08` and `yt-dlp --list-extractors` was checked for this extractor entry.
- RedTube extractor source: https://github.com/yt-dlp/yt-dlp/blob/master/yt_dlp/extractor/redtube.py
