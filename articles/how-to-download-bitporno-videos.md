...
Site name: bitporno
Domain: bitporno.sx
Article path: content/site-articles/bitporno-sx.md
...
```text
Attempted URL: http://bitporno.sx/
...
```bash
URL="https://bitporno.sx/path/to/video"
...
```bash
page_url="https://bitporno.sx/path/to/video"
...
```bash
ffmpeg -i "input-from-bitporno-sx.ext" -map 0 -c copy -movflags +faststart "bitporno-sx-fixed.mp4"
```
