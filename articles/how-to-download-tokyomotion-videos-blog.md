# How to Download TokyoMotion Videos | Technical Analysis of Generic Embed Patterns, Manifests, and Download Methods

This research document provides a technical analysis of TokyoMotion video download detection for the hostname `tokyomotion.net`. The site is treated as `generic_embed_or_manifest`: a downloader should discover the active runtime media source from the authorized page, iframe, player configuration, manifest request, or HTML5 media element instead of assuming a dedicated extractor, fixed CDN hostname, or stable direct-file route.

This article did not visit explicit media pages and did not download media from `tokyomotion.net`. The implementation guidance is based on the provided site inventory, local tool checks, existing project article conventions, and public documentation for browser media elements, embedded players, HLS/DASH manifests, `yt-dlp`, FFmpeg, and browser network inspection.

But first...

## TokyoMotion Video Downloader Chrome Extension

If you prefer a simpler one-click method, check out the TokyoMotion Video Downloader browser extension.

TokyoMotion Downloader is a browser extension built for users who want a cleaner way to save accessible TokyoMotion videos without falling back to developer tools, network tabs, or command-line utilities. It runs in the active browser session, detects supported iframe, HTML5, JW Player, Video.js, KVS/kt_player, HLS, DASH, MP4, and WebM sources, and exports the final result as a usable local file when normal download tooling can process the stream.

- Save supported TokyoMotion videos from pages you are authorized to access
...
👉 [Click here to try it free](https://serp.ly/tokyomotion-downloader?via=github&utm_source=github&utm_medium=piggyback&utm_campaign=howtodownloadvideosagent)
...
```text
Canonical hostname: tokyomotion.net
Same-site URL prefix: https://tokyomotion.net/
Allowed first-party hosts: tokyomotion.net, www.tokyomotion.net
Hostname regex: ^(?:www\.)?tokyomotion\.net$
```

The conservative architecture model is:

1. Start with the authorized TokyoMotion page on `https://tokyomotion.net/`.
...
```json
{
  "site_name": "TokyoMotion",
  "hostname": "tokyomotion.net",
...
No fixed TokyoMotion CDN domain is asserted here. Generic media pages can shift between first-party hosts, storage hosts, player vendors, and signed delivery URLs. A downloader should record the actual media host discovered at runtime, not a guessed host.
...
For TokyoMotion, the downloader should look for common generic player shapes without claiming they are always present on `tokyomotion.net`:
...
The detector should preserve the original TokyoMotion page URL as the `Referer` for iframe, manifest, and direct media requests unless runtime evidence proves a different referer is required.
...
Do not strip query parameters from TokyoMotion media URLs. Signed URLs commonly put access tokens, expiry timestamps, signatures, policy data, or cache keys in the query string. A shortened URL may look cleaner but fail with HTTP 403.

---

## 4. Stream Formats and CDN Analysis

For TokyoMotion, the supported stream classes should be discovered in this order:
...
Direct file delivery is simpler but still context-sensitive. A direct `.mp4` or `.webm` URL from a TokyoMotion page can require cookies, range requests, signed query strings, and the original referer.
...
Do not construct CDN URLs for TokyoMotion. Record the actual hostnames the browser requests during playback and classify each request by evidence:

- First-party HTML or API request under `tokyomotion.net`
...
```bash
printf '%s
' "$TOKYOMOTION_NET_MEDIA_URL" | node -e '
...
Store the discovered hostname for diagnostics, but do not hard-code it as the TokyoMotion CDN.
...
```bash
TOKYOMOTION_NET_PAGE_URL="$(pbpaste)"

yt-dlp \
  --force-generic-extractor \
  --list-formats \
  "$TOKYOMOTION_NET_PAGE_URL"
```
...
yt-dlp \
  ...
  --add-headers "Referer:$TOKYOMOTION_NET_PAGE_URL" \
  --add-headers "Origin:https://tokyomotion.net" \
...
```bash
TOKYOMOTION_NET_PLAYER_URL="$(pbpaste)"
...
yt-dlp \
  ...
  -o "TokyoMotion - %(title).200B.%(ext)s" \
  "$TOKYOMOTION_NET_PAGE_URL"
```
...
```bash
TOKYOMOTION_NET_MANIFEST_URL="$(pbpaste)"

yt-dlp \
  --cookies-from-browser chrome \
  --add-headers "Referer:$TOKYOMOTION_NET_PAGE_URL" \
  --add-headers "User-Agent:Mozilla/5.0" \
  "$TOKYOMOTION_NET_MANIFEST_URL"
```
...
ffmpeg \
  -headers "Referer: $TOKYOMOTION_NET_PAGE_URL\r\nUser-Agent: Mozilla/5.0\r\n" \
  -protocol_whitelist file,http,https,tcp,tls,crypto \
  -i "$TOKYOMOTION_NET_MANIFEST_URL" \
  -map 0 \
  -c copy \
  "tokyomotion-net.mp4"
```
...
ffmpeg \
  ...
  -map 0 \
  -c copy \
  "tokyomotion-net-dash.mkv"
```

### 6.3 Direct MP4 or WebM

```bash
TOKYOMOTION_NET_MEDIA_URL="$(pbpaste)"

ffmpeg \
  -headers "Referer: $TOKYOMOTION_NET_PAGE_URL\r\nUser-Agent: Mozilla/5.0\r\n" \
  -i "$TOKYOMOTION_NET_MEDIA_URL" \
  -map 0 \
  -c copy \
  "tokyomotion-net.mp4"
```
...
When the TokyoMotion browser request includes cookies or an origin header, copy only the required request context from an authorized session.

```bash
TOKYOMOTION_NET_REQUEST_HEADERS="$(pbpaste)"

ffmpeg \
  -headers "$TOKYOMOTION_NET_REQUEST_HEADERS" \
  -referer "$TOKYOMOTION_NET_PAGE_URL" \
...
```bash
ffmpeg -i "$TOKYOMOTION_NET_MEDIA_URL" -map 0 -c copy "tokyomotion-net.mkv"
ffmpeg -i "$TOKYOMOTION_NET_MEDIA_URL" -c:v libx264 -crf 20 -preset veryfast -c:a aac "tokyomotion-net-compatible.mp4"
```
...
The browser remains the source of truth for generic TokyoMotion pages. Open DevTools before playback, enable Preserve log, start playback, then filter for:
...
jq -r '.log.entries[] | [.request.method, .response.status, .response.content.mimeType, .request.url] | @tsv' capture.har \
  | rg -i 'tokyomotion\.net|m3u8|mpd|\.ts|\.m4s|\.mp4|\.webm|jwplayer|videojs|kt_player|flashvars|signature|policy|key-pair-id|x-amz'
```
...
```bash
streamlink "$TOKYOMOTION_NET_MANIFEST_URL" best -o "tokyomotion-net.ts"
streamlink --stream-url "$TOKYOMOTION_NET_MANIFEST_URL" best
```
...
```bash
N_m3u8DL-RE "$TOKYOMOTION_NET_MANIFEST_URL" --save-name "tokyomotion-net"
```
...
```bash
curl -I -L \
  -H "Referer: $TOKYOMOTION_NET_PAGE_URL" \
...
Build the TokyoMotion downloader as a layered detector:

1. Accept a browser-active `tokyomotion.net` URL from the user.
2. Confirm the normalized hostname is `tokyomotion.net` or `www.tokyomotion.net`.
...
The downloader should never report TokyoMotion as supported just because the hostname matches `tokyomotion.net`. Support should be tied to runtime evidence: a playable manifest, a direct media file, or a player configuration that resolves in the authorized browser session.
...
Common TokyoMotion failure cases:
...
```bash
yt-dlp --verbose --force-generic-extractor -F "$TOKYOMOTION_NET_PAGE_URL"
yt-dlp --cookies-from-browser chrome --verbose -F "$TOKYOMOTION_NET_PAGE_URL"
ffprobe -hide_banner "$TOKYOMOTION_NET_MANIFEST_URL"
```
